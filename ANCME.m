function [estIF, estIA, estMode] = ANCME(signal, fs, iniIF, beta, tol)
%
% This code implements the ANCME algorithm (https://github.com/HauLiang/ANCME)
%
% This code is based on the codes from the following paper
...
% [2] Liang H, Ding X, Jakobsson A, et al. Adaptive Variational Nonlinear Chirp Mode Decomposition. ICASSP, 2022.
% Please check the accompanying license and the license of [1] before using. 
%
%
% Inputs:
%    signal:   sampled time-domain signal
...
iternum = 250;     % the maximum allowable iterations
...
while ( sDif > tol &&  iter <= iternum )
    ...
    x = preliminary_model(A, D, signal(:), K, N); x = x';
    ...
    for i = 1:K
        ...
        Modeset_iter(i,:,iter+1) = uk(i,:).*cosm(i,:) + vk(i,:).*sinm(i,:);
